...
export const ListOfQA = () => {
  ...
  return (
    ...
    {state.list.length >0 && <h5>List Of all the Questions</h5>}
      {state.list.map((e:{id:string,question:string,answer:string},index:string) => {
        return (
          <div className={classes.main} key={index}>
            <form>
              ...
              ></input>
              <br />
              <button className={classes.del} type='button' onClick={() => onDeleteHandler(e.id)}>Delete</button>
              <br />
            </form>
          </div>
        );
